...
async function sendLoginCode() {
  ...
  try {
    ...
    const result = await $fetch<UserBearerToken>(`${config.public.apiBase}/auth/login/callback`, {
      method: 'POST',
      body: {
        email: props.email,
        code: passcode.value
...
<template>
  <form class="passcode-container" @submit.prevent="sendLoginCode" novalidate>
    <a id="passcode-text">ログイン</a>
    ...
    <button id="back-to-emailbox" @click="emit('back')">
      メールアドレス入力へ戻る
    </button>
  </form>
</template>
